...
export class SchoolRoutes extends RouteConfig {
  ...
  public configureRoutes() {
    this.app
      .route(`/${this.baseUrl}/board-types`)
      .get([
        AuthMiddleware.verifyToken([Role.RegularUser]),
        SchoolController.getBoardTypes,
      ]);
    this.app
      .route(`/${this.baseUrl}/board-type/:id`)
      .get([
        AuthMiddleware.verifyToken([Role.RegularUser]),
        SchoolController.getBoardTypeById,
      ]);
    this.app
      .route(`/${this.baseUrl}/board-type-by-title/:title`)
      .get([
        AuthMiddleware.verifyToken([Role.RegularUser]),
    ...
    this.app
      .route(`/${this.baseUrl}/room/:id`)
      .get([
        AuthMiddleware.verifyToken([Role.RegularUser]),
        SchoolController.getRoomById,
      ]);
    this.app
      .route(`/${this.baseUrl}/room`)
      .post([
        AuthMiddleware.verifyToken([Role.RegularUser]),
